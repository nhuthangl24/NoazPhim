...
export default function TrailerButton({ trailerUrl }) {
  ...
  // Extract YouTube ID if it's a standard youtube.com/watch?v= or youtu.be/ link
    ...
    <>
      ...
      {isOpen && (
        <div className="modal-overlay" onClick={() => setIsOpen(false)} style={{ zIndex: 9999 }}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '800px', width: '90%', padding: '0', background: '#000', overflow: 'hidden' }}>
            <button className="modal-close" onClick={() => setIsOpen(false)} style={{ top: '-40px', right: '0', color: '#fff', fontSize: '1.5rem' }}>✕</button>
